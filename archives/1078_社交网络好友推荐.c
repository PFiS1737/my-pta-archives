...
int main() {
  ...
  for (int i = 0; i < N; i++) {
    scanf("%s", mans[i].name);
    strcpy(mans[i].result, "");

    for (int j = 0; j < M; j++) {
      scanf(" %c", &mans[i].answer[j]);
    }
  ...
  for (int i = 0; i < N; i++) {
    ...
    for (int j = 0; j < N; j++) {
      ...
      int count = 0;
      for (int k = 0; k < M; k++) {
        if (mans[i].answer[k] == mans[j].answer[k]) {
          count++;
        }
      }

      if (count >= mans[i].Q) {
        strcat(mans[i].result, is_first ? (is_first = 0, "") : ",");
        strcat(mans[i].result, mans[j].name);
      }
    }

    if (is_first) {
      strcpy(mans[i].result, "NONE!");
    }
  }

  for (int i = 0; i < N; i++) {
    printf("%-*s:%s\n", max_name_length, mans[i].name, mans[i].result);
  }
